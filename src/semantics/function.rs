...
impl<'a> FunctionContext<'a> {
    ...
    pub fn analyze_function(&self, fun: &'a FunDef) -> FrontendResult<()> {
        ...
        self.enter_block(&fun.body, env).accumulate_errors_in(&mut errors);

        if errors.is_empty() { Ok(()) } else { Err(errors) }
    }

    fn enter_block(&self, block: &'a Block, parent_env: VarEnv<'a>) -> FrontendResult<()> { // todo
        let mut errors = vec![];
        ...
        for st in &block.stmts {
            match &st.inner {
                ...
                Block(bl) => {
                    ...
                    self.enter_block(&bl, new_env).accumulate_errors_in(&mut errors);
                },
                Decl{var_type, var_items} => {
                    self.global_ctx.check_local_var_type(&var_type).accumulate_errors_in(&mut errors);
                    for (id, init_expr) in var_items {
                        ...
                        if let Some(init_expr) = init_expr {
                            ...
                            self.check_expression_check_type(&init_expr, &var_type, &cur_env).accumulate_errors_in(&mut errors);
                        }
                    }
                },
                Error => unreachable!(),
    ...
    fn check_expression_check_type(&self, expr: &'a Expr, expected_expr_type: &'a Type, cur_env: &VarEnv<'a>) -> FrontendResult<()> {
        let expr_type = self.check_expression_get_type(expr, cur_env)?;
        self.global_ctx.check_types_compatibility(expected_expr_type, &expr_type)
    }

    fn check_expression_get_type(&self, _expr: &'a Expr, _cur_env: &VarEnv<'a>) -> FrontendResult<Type> {
        Err(vec![]) // todo
    }
}
